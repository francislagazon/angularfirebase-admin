...
import { USERS_SECRET_PATH } from './../user/user.interface'
...
export class Base {
    ...
    getSecretKey(uid: string): firebase.Promise<any> {
        ...
        return this.root.child(USERS_SECRET_PATH).child(uid).once('value')
            .then(snap => {
                let v = snap.val();
    ...
    setSecretKey(uid: string): firebase.Promise<any> {
        ...
        return this.root.child(USERS_SECRET_PATH).child(uid).set(secretkey)
            .then(() => secretkey)
